...
Arguments:
  ...
  --random_dates : Optional flag to generate random dates over ~ num_obs*7 days baseline
...
def parse_args():
    ...
    parser.add_argument("--planets", type=str, required=True,
                        help="Semicolon-separated list: 'mass,period,ecc,inc' per planet (empty string for none)")
    ...
    parser.add_argument("--time_step", type=str, default="10d0h",
                        help="Spacing between observations: e.g. '3d0h' (ignored if date_list supplied or random_dates enabled)")
    ...
    parser.add_argument("--random_dates", action="store_true",
                        help="Generate random observation dates over ~ num_obs*7 days baseline")
    return parser.parse_args()

def parse_planets(planets_str):
    planets = []
    if planets_str.strip() == "":
        return planets  # No planets
...
def simulate():
    ...
    if args.date_list:
        ...
        if args.num_obs is None:
            raise ValueError("--num_obs is required if using --random_dates")
        base_time = datetime(2025, 2, 6, 0, 0, 0)
        total_span_days = args.num_obs * 7
        obs_times_dt = sorted([base_time + timedelta(days=random.uniform(0, total_span_days)) for _ in range(args.num_obs)])
    else:
        if args.num_obs is None:
    ...
    if args.date_list or args.random_dates:
        # Pass dates directly
