...
# --- SETUP THE SINGLE, WIDE MASTER GRID FOR ALL PROCESSING ---
full_wave_min = np.exp(np.min([np.log(w[0]) for w, _ in instrument_grids]))
full_wave_max = np.exp(np.max([np.log(w[-1]) for w, _ in instrument_grids]))
d_log_wave = instrument_grids[0][1]
num_points = int(np.ceil((np.log(full_wave_max) - np.log(full_wave_min)) / d_log_wave)) + 1
full_log_wave_grid = np.linspace(np.log(full_wave_min), np.log(full_wave_max), num_points)
full_wave_grid = np.exp(full_log_wave_grid)

# Define interpolators on full original stellar and FTS data
fts_interp = lambda x: np.interp(x, fts_wave, fts_flux, left=fts_flux[0], right=fts_flux[-1])
star_interp_master = lambda x: np.interp(x, star_wave, star_flux, left=star_flux[0], right=star_flux[-1])

# ---------- Generate synthetic observations ON THE FULL SPECTRUM----------
print(f"\n=== GENERATING {NUM_OBS} OBSERVATIONS ===")
for i in range(NUM_OBS):
    print(f"\n--- Simulation {i+1}/{NUM_OBS} ---")
    obs_time = obs_times[i]
    ...
    total_shift = total_rv + rv_activity - barycorr_ms
    print(f"Total RV: {total_rv:.2f} m/s, Barycentric: {barycorr_ms:.2f} m/s, Final shift: {total_shift:.2f} m/s")
    doppler_factor = 1.0 + total_shift / SPEED_OF_LIGHT

    wave_grid_rest = full_wave_grid / doppler_factor
    star_flux_full = star_interp_master(wave_grid_rest)
    fts_flux_full = fts_interp(full_wave_grid)
    model_full = star_flux_full * fts_flux_full
    
    flux_conv_full = convolve_IP_on_uniform_grid(model_full, d_log_wave, ip_widths[i], ip_type=args.ip_type, asymmetry=args.asymmetry, gamma=args.gamma,)

    wave_orders_final = []
    flux_orders_final = []

    for wave_k_instrument, _ in instrument_grids:
        start_idx = np.searchsorted(full_wave_grid, wave_k_instrument[0])
        wave_order_eff = wave_k_instrument[VIPER_IP_HALF_SIZE : -VIPER_IP_HALF_SIZE]
        
        flux_slice_start = start_idx
        flux_slice_end = flux_slice_start + len(wave_order_eff)
        flux_order_eff = flux_conv_full[flux_slice_start : flux_slice_end]
        
        if len(wave_order_eff) != len(flux_order_eff):
            print(f"Warning: Wavelength and flux array length mismatch in science order. Skipping.")
            continue

        wave_orders_final.append(wave_order_eff)
        flux_orders_final.append(flux_order_eff)
...
# --- Independent Template Generation (NO IP CONVOLUTION, WITH TRIMMING) ---
if args.template:
    print(f"\n=== GENERATING TEMPLATE ===")
    first_obs_time = obs_times[0]
    obstime_tpl = Time(first_obs_time, scale='utc')
    ...
    wave_grid_rest_tpl = full_wave_grid / doppler_factor_tpl
    model_tpl_full = star_interp_master(wave_grid_rest_tpl)
    ...
    for wave_k_instrument, _ in instrument_grids:
        start_idx = np.searchsorted(full_wave_grid, wave_k_instrument[0])
        end_idx = np.searchsorted(full_wave_grid, wave_k_instrument[-1]) + 1
        
        wave_order_tpl = full_wave_grid[start_idx:end_idx]
        flux_order_tpl = model_tpl_full[start_idx:end_idx]

        wave_order_eff = wave_order_tpl[VIPER_IP_HALF_SIZE : -VIPER_IP_HALF_SIZE]
        flux_order_eff = flux_order_tpl[VIPER_IP_HALF_SIZE : -VIPER_IP_HALF_SIZE]

        w_orders_tpl.append(wave_order_eff)
        f_orders_tpl.append(flux_order_eff)
        
    write_default_fits(OUTPUT_TEMPLATE_FILE, w_orders_tpl, f_orders_tpl, first_obs_time.isoformat(timespec='milliseconds'))
    print(f" -> {OUTPUT_TEMPLATE_FILE}")

# =============================================================================
# # --- Independent Template Generation (MODIFIED AS PER YOUR EXACT INSTRUCTIONS) ---
# if args.template:
#     print(f"\n=== GENERATING TEMPLATE ===")
#     first_obs_time = obs_times[0]
#     obstime_tpl = Time(first_obs_time, scale='utc')
...
#     f_orders_tpl = []
#     
#     # Interpolate onto the *fixed instrument grids* for each order
#     for k, (wave_k, d_log_wave_k) in enumerate(instrument_grids):
#         flux_k = star_interp_tpl(wave_k)
#         w_orders_tpl.append(wave_k)
#         f_orders_tpl.append(flux_k)
...
# =============================================================================
# # --- Independent Template Generation (MODIFIED TO MATCH TRIMMING ONLY) ---
# if args.template:
#     print(f"\n=== GENERATING TEMPLATE ===")
#     first_obs_time = obs_times[0]
#     obstime_tpl = Time(first_obs_time, scale='utc')
...
#     # Interpolate onto the *fixed instrument grids* for each order
#     for k, (wave_k, d_log_wave_k) in enumerate(instrument_grids):
#         # 1. Interpolate the stellar spectrum
#         flux_k = star_interp_tpl(wave_k)
#         
#         # 2. Trim the wavelength array to match the science data
#         wave_k_eff = wave_k[VIPER_IP_HALF_SIZE : -VIPER_IP_HALF_SIZE]
#         
#         # 3. Trim the flux array to match the wavelength array
#         flux_k_eff = flux_k[VIPER_IP_HALF_SIZE : -VIPER_IP_HALF_SIZE]
# 
#         # Append the trimmed arrays
#         w_orders_tpl.append(wave_k_eff)
#         f_orders_tpl.append(flux_k_eff)
#         
#     write_default_fits(OUTPUT_TEMPLATE_FILE, w_orders_tpl, f_orders_tpl, first_obs_time.isoformat(timespec='milliseconds'))
#     print(f" -> {OUTPUT_TEMPLATE_FILE}")
# =============================================================================
