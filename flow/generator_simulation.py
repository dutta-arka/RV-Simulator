...
# FIX: Use arange to preserve the exact d_log_wave step size
full_log_wave_grid = np.arange(np.log(full_wave_min), np.log(full_wave_max) + d_log_wave, d_log_wave)
...
for i in range(NUM_OBS):
    ...
    write_default_fits(output_path, wave_orders_final, flux_orders_final, date_obs_str)
    print(f" -> {output_path}")
...
if args.template:
    ...
    output_path_template = os.path.join(args.output_dir, OUTPUT_TEMPLATE_FILE)
    write_default_fits(output_path_template, w_orders_tpl, f_orders_tpl, first_obs_time.isoformat(timespec='milliseconds'))
    print(f" -> {output_path_template}")
